...
import { addChat } from '../../../app/features/Chat/chatReducer'
import { useChatListSelector } from '../../../app/features/Chat/chatListSelectors'
import { Dialog } from '@mui/material'
import { useState } from 'react'
import { addChatWithFirebase } from '../../../app/features/Chat/middleware/middleware'
...
const Chatboard = () => {
    ...
    const handleAddChat = async (e) => {
        e?.preventDefault()
        if (chatForm.email === profiler.email) {
        ...
        for (const chatItem in chatList) {
            if (chatList[chatItem].email === chatForm.email) {
                setDialogClose((prev) => !prev)
                setChatForm({
                    chatName: '',
                    email: '',
        ...
        setDialogClose((prev) => !prev)
        const chatName = chatForm.chatName
        const email = chatForm.email
        setChatForm({
            chatName: '',
            email: '',
        })
        await addChatWithFirebase(profiler, chatName, email)
    }
    return (
        <div className="chats">
            ...
            <div className="chat_menu">
                ...
                <button
                    className="chat_menu__item"
                    onClick={() => setDialogClose((prev) => !prev)}
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
            ...
            <Dialog open={dialogClose}>
                <form className="add_chat" onSubmit={handleAddChat}>
                    <h3>
                        Введите название чата и электронную почту пользователя
                    </h3>
                    <input
                        type="text"
